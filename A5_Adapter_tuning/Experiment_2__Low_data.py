...
adapter_size = 32
batch_size = 2
save_path = f"../Saved_weights/EXP2/Adapters/{MODEL_NAME}"
######################################################
...
for train_size in dataset_sizes:
    ...
    print("Adapter block size ==> ", adapter_size)
    adapter_blocks, tuning_logs = Adapter_Tuning.get_tuned_adapters(
        training_dataloader,
        mt,
        adapter_dim = adapter_size,
        num_epochs = 1,
        # limit = 10
    )

    os.makedirs(save_path, exist_ok = True)
    torch.save(adapter_blocks, f"{save_path}/adapter_dim__{adapter_size}____data_{train_size}.pth")
    ...
    with open(f"{save_path}/logs_adapter_dim__{adapter_size}__data_{train_size}.json", "w") as f:
        json.dump({
